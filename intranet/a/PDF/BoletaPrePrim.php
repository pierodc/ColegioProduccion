...
do{	
	...
	if(file_exists($foto) or true){
		...
		$pdf->Image('../../../img/solcolegio.jpg', 25, 10, 30);
		
		if($_GET['Nivel'] == 'Preescolar')
			$pdf->Image('../../../img/Boleta.jpg', 40, 80, 150);
		else
			$pdf->Image('../../../img/BoletaPrimaria.jpg', 40, 80, 150);
		...
		if($_GET['Nivel'] == 'Preescolar')
			$TipoBoleta = "Educación inicial";
		else
			$TipoBoleta = "Educación Primaria";
	
		
		$pdf->Cell($MargDer); $pdf->Cell(200 , 5 , $TipoBoleta , $borde , 1 , 'C'); 
		...
		if($Docente[3]>''){
			$pdf->Cell($MargDer+$AnchoFoto+25); $pdf->Cell(44 , 7 , 'Aux: '.$Docente[3] , $borde , 1 , 'L'); }
