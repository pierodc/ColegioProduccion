...
//$NombreArchivo = $ArchivoDe.'_'.date('Y').'_'.date('m').'_'.date('d').'_L'.$Lote.'_D'.$DiaAbono.'.txt';
$NombreArchivo = $ArchivoDe.'_'.date('Y').'_'.date('m').'_'.date('d').'_L'.$Lote.'_D'.$DiaAbono.'.txt';	
...
// Encabezado
		$Io = 1;
		$Io = Chk($swP,$Io, "1BAMRVECA    00000000000".$Lote."NOMIN"); 
...
// Renglones
	...
	while($row_Empleados = $RS_Empleados->fetch_assoc()){ 
		...
		if($Monto > 0){
			...
			if (substr($row_Empleados['NumCuentaA'] , 0 , 4) == "0105")
				$Io = Chk($swP,$Io,  "1");
			else
				$Io = Chk($swP,$Io,  "3");
...
if($ArchivoDe == 'Pago_extra' and false){ // Archivo Nomina Pago Extra
...
if($ArchivoDe == 'Pago_extra2' and false){ // Archivo Nomina Pago Extra
