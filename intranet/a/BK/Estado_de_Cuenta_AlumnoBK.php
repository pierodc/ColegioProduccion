...
if ($CodidoBuscando > 0){
	...
	$row_RS_sql= mysql_fetch_assoc($RS_sql); //echo $row_RS_sql['CodigoClave'];
...
$sqlAux = mysql_query("SELECT CodigoAlumno 
						FROM Alumno 
						WHERE CodigoClave = '".$_GET['CodigoPropietario']."'", $bd) or die(mysql_error());
$Aux = mysql_fetch_assoc($sqlAux);
$CodigoAlumno = $Aux['CodigoAlumno'];

$sqlAux = mysql_query("SELECT CodigoCurso 
						FROM AlumnoXCurso 
						WHERE CodigoAlumno = '$CodigoAlumno' 
						AND Ano = '$AnoEscolar'", $bd) or die(mysql_error());
$Aux = mysql_fetch_assoc($sqlAux);
$CodigoCurso  = $Aux['CodigoCurso'];
//echo $CodigoCurso;
//$CodigoCursoProxAno  = $Aux['CodigoCurso']; // AJUSTAR PARA ANO 2012-13

$sqlAux = mysql_query("SELECT NivelCurso , CodigoCurso
						FROM Curso 
						WHERE CodigoCurso = '$CodigoCurso' ", $bd) or die(mysql_error());
$Aux = mysql_fetch_assoc($sqlAux);
$NivelCursoActual  = $Aux['NivelCurso'];
	

// INSCRIBE un alumno y crea facturas  OJO VERIFICAR UTILIDAD
/*
if (isset($_GET['CodigoPropietario']) and isset($_GET['Inscribir']) and false) {
	
	$sql = "";
		
	// Agrega Facturas de Matriculacion del curso 
	$Fecha_Ins = date('y').'-08-31';
	//$Fecha_Ins = '2011-08-31';
	
	$sql = "";
	$sql = "INSERT INTO ContableMov (CodigoPropietario, Fecha, FechaIngreso, FechaValor, SWValidado, RegistradoPor, Referencia, ReferenciaMesAno, Descripcion, MontoDebe) ";//".date('y')."
	$sql.= "SELECT $CodigoAlumno, '$Fecha_Ins', '$Fecha_Ins', '$Fecha_Ins', 1, '$MM_Username', Asignacion.Codigo, 'Ins 11',Asignacion.Descripcion, Asignacion.Monto FROM Asignacion WHERE (NivelCurso = '$NivelCurso' OR CodigoCurso=0) AND Periodo = 'I' AND SWActiva = 1 ORDER BY CodigoCurso, Codigo";
	//	echo $sql."<br><br>";
	...
	$sql = "SELECT * FROM AsignacionXAlumno WHERE CodigoAlumno='$CodigoAlumno' AND Ano_Escolar='$AnoEscolarProx'";
	$RS_sql = mysql_query($sql, $bd) or die(mysql_error());
	...
	$sql.= "SELECT $CodigoAlumno, Asignacion.Codigo, 0, '$AnoEscolarProx' FROM Asignacion WHERE (NivelCurso = '$NivelCurso') AND Periodo = 'M' AND SWActiva = 1 ORDER BY Codigo";
	//echo $sql."<br><br>";
	$RS_sql = mysql_query($sql, $bd) or die(mysql_error()); 
	//}  if($totalRows_RS_sql==0)
	//}
...
if (isset($_GET['SWAgostoFraccionado'])) {
	...
	$query_RS_Alumnos = sprintf("UPDATE Alumno SET SWAgostoFraccionado = '".$_GET['SWAgostoFraccionado']."' 
...
if (isset($_POST['Actividad']) and $_POST['CodigoAsignacion3'] > 0) {
	$sql = "SELECT * FROM Asignacion WHERE Codigo = '".$_POST['CodigoAsignacion3']."'";
	$RS_sql = mysql_query($sql, $bd) or die(mysql_error());
    $row_RS_sql = mysql_fetch_assoc($RS_sql);
	$MontoDebe = $row_RS_sql['Monto'];
	$Descripcion = $row_RS_sql['Descripcion'];

	$sql = "INSERT INTO ContableMov (CodigoPropietario, Fecha, FechaIngreso, FechaValor, SWValidado, RegistradoPor, Referencia, Descripcion, MontoDebe) ";
	$sql.= "( SELECT $CodigoAlumno, '".$_POST['FechaActividad']."', NOW(), '".$_POST['FechaActividad']."', 1, '".$_COOKIE['MM_Username']."', ";
	$sql.= "'".$_POST['CodigoAsignacion3']."', '$Descripcion', '$MontoDebe'   ";
	$sql.= ") ";
	$sql.= "";echo $Insp ?  $sql." (5)<br>" : "";
	$RS_sql = mysql_query($sql, $bd) or die(mysql_error());
}
...
if (isset($_POST['EventualDescripcion'])) {
	$sql = "";
	$sql = "INSERT INTO ContableMov (CodigoPropietario, Fecha, FechaIngreso, FechaValor, SWValidado, RegistradoPor, ReferenciaMesAno, Referencia, Descripcion, MontoDebe, SWiva) ";
	$sql.= " VALUES ($CodigoAlumno,  '".$_POST['FechaEventual']."', NOW(),  '".$_POST['FechaEventual']."', 1, '".$_COOKIE['MM_Username']."', ";
	$sql.= "'".$_POST['EventualReferencia']."' , '".$_POST['EventualDescripcion']."', '".$_POST['EventualDescripcion']."'  , ";
	$sql.= "'".$_POST['EventualMonto']."','".$_POST['SWiva']."' )";
	echo $Insp ?  $sql." (8)<br>" : "";
	$RS_sql = mysql_query($sql, $bd) or die(mysql_error());
}
...
$RS_Mov_Pagos = mysql_query($query_RS_Mov_Pagos, $bd) or die(mysql_error());
$row_RS_Mov_Pagos = mysql_fetch_assoc($RS_Mov_Pagos);
$totalRows_RS_Mov_Pagos = mysql_num_rows($RS_Mov_Pagos);
...
$RS_ContableMov_Procesando = mysql_query($query_RS_ContableMov_Procesando, $bd) or die(mysql_error());
$row_RS_ContableMov_Procesando = mysql_fetch_assoc($RS_ContableMov_Procesando);
$totalRows_RS_ContableMov_Procesando = mysql_num_rows($RS_ContableMov_Procesando);
...
$RS_ContableMov = mysql_query($query_RS_ContableMovAUX, $bd) or die(mysql_error());
$row_RS_ContableMov = mysql_fetch_assoc($RS_ContableMov);
...
/*		
function ResumenXX($ReferenciaMesAno, $CodigoPropietario, $database_bd, $bd) {	
	// DESACTIVADO APROVECHAR CALCULO DEUDA PENDIENTE	   
	mysql_select_db($database_bd, $bd);
	$query_RS_Mov_Mes = "SELECT * FROM ContableMov, Recibo 
						  WHERE ContableMov.CodigoRecibo = Recibo.CodigoRecibo 
						  AND ContableMov.CodigoPropietario = $CodigoPropietario 
						  AND ContableMov.ReferenciaMesAno = '$ReferenciaMesAno' 
						  ORDER BY Codigo ASC"; 
	$RS_Mov_Mes = mysql_query($query_RS_Mov_Mes, $bd) or die(mysql_error());
	$row_RS_Mov_Mes = mysql_fetch_assoc($RS_Mov_Mes);
	$totalRows_RS_Mov_Mes = mysql_num_rows($RS_Mov_Mes);
	$MontoAux = 0;
	do {
		if($row_RS_Mov_Mes["SWCancelado"] == 1 ) {
			echo "<b>";}
		echo substr($row_RS_Mov_Mes["Descripcion"],0,5);
		$MontoAux = $MontoAux + $row_RS_Mov_Mes["MontoDebe"];
		
		if($row_RS_Mov_Mes["SWCancelado"] == 1 ) {
			echo "</b> ".substr($row_RS_Mov_Mes["FechaRecibo"],8,2)."/".substr($row_RS_Mov_Mes["FechaRecibo"],5,2);}
		echo "<br>";
	} while ($row_RS_Mov_Mes = mysql_fetch_assoc($RS_Mov_Mes));;
	echo Fnum($MontoAux);
}
*/
 $$StopResumen = false;
...
$RS_ = mysql_query($sql, $bd) or die(mysql_error());
$row_Curso_Actual = mysql_fetch_assoc($RS_);
...
//	else{						  
                    ...
                    <?php } while ($row_RS_ContableMov_Procesando = mysql_fetch_assoc($RS_ContableMov_Procesando)); ?>
        </table>
...
<?php
mysql_free_result($RS_ContableMov_Procesando);

mysql_free_result($Recibos);
?>
