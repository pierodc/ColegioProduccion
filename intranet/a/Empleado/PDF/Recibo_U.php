...
$query_RS_Empleados = "SELECT * FROM Empleado WHERE 
					   SW_activo=1 
					   $add_sql 
					   ORDER BY Apellidos, Nombres LIMIT 0 , 200 ";
...
if(!isset($_GET['CodigoEmpleado'])){
	$sql_Pago_Extra = "UPDATE Empleado SET Pago_extra = ''";
...
*/
	...
	$sql = "SELECT * FROM Empleado_Pago WHERE 
			...
			Codigo_Quincena LIKE '%$Ano%' AND 
			Concepto LIKE '+SueldoBase'
			ORDER BY Empleado_Pago.Codigo_Quincena ASC";
	$RS_Pagos = mysql_query($sql, $bd) or die(mysql_error());
	

	while($row_Pagos = mysql_fetch_assoc($RS_Pagos)){
		extract($row_Pagos);
		
		$Pago[substr($Codigo_Quincena , 5 , 2)][substr($Codigo_Quincena , 8 , 1)][Codigo_Quincena] = $Codigo_Quincena;
		$Pago[substr($Codigo_Quincena , 5 , 2)][substr($Codigo_Quincena , 8 , 1)][Monto] = $Monto;
...
//foreach($Pago as $Pagos)
	echo "aaa". $Pago[2][1][Codigo_Quincena];
echo "</pre>";
	...
	if($MontoBase > 0){
		...
		$pdf->SetFont('Arial','',12);
		$pdf->Cell(170,$linea, $Concepto ,0,0,'R'); 
		//$pdf->Write($linea, $Concepto );
		
		$Concepto = 'Bs.'.Fnum($MontoPago).'';
		$pdf->SetFont('Arial','B',12);
		$pdf->Cell(25,$linea, $Concepto ,0,1,'R'); 
		//$pdf->Write($linea, $Concepto );  $pdf->Ln();
		...
		*/
		...
		$pdf->Ln($linea*2);
		$pdf->Cell(195,$linea, "fi: ".DDMMAAAA($row_RS_Empleados['FechaIngreso']).' -> '.$DiasPagados.'d / '."sb: ".$row_RS_Empleados['SueldoBase'] , 0 , 0,'R');
