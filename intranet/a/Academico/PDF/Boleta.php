...
do {
	...
	foreach (array(1, 2, 3) as $_Lapso) { // cada LAPSO
		foreach (array('-70','-30','-Def','-BConduc','i','mp') as $_Eval) { // Cada Evaluacion
			...
			if($_totalRows > 0)
			foreach ( $Sec_01_13 as $fila_x ) { // Cada Materia
				...
				if(  $_row_Nota['n'.$fila_x] > 0 and 
					 $_row_Nota['n'.$fila_x] <= 20 and 
					 $Matriz[materia][$fila_x] > "" and 
					 $_Eval == '-Def' ) { 
	...
	$pdf->Cell(50);
	foreach (array('1er ', '2do ', '3er ') as $nom) { // Titulo Trimestre    , '3er '
		$pdf->Cell(40,$linea, $nom.'Momento' ,1,0,'C'); 		
	}
	...
	foreach (array('1er ', '2do ', '3er ') as $nom) { // Titulo Evaluaciones de cada Trim   , '3er '
		foreach (array('70%','30%','Def','Pos','I') as $eva) {
			$pdf->Cell(8,$linea, $eva ,1,0,'C'); 		
	}	} 
	
	// Titulo Evaluaciones FINAL
	foreach (array('Def','Pos','I') as $eva) {      
			$pdf->Cell(8,$linea, $eva ,1,0,'C'); 		
	}
	$pdf->Ln();
	...
	foreach ( $Sec_01_13 as $fila_x ) { 
		...
		if($Matriz[materia][$fila_x] > "" or true){
			...
			foreach (array(1, 2, 3) as $_Lapso) { // 
				foreach (array('-70','-30','-Def','Pos','i') as $_Eval) {
					...
					if($_Eval == "-Def") {
						$b = 'B';
						if($Matriz[$_Lapso]['-BConduc'][$fila_x]==1) 
							$BConduc = '•';
						if($Matriz[$_Lapso]['-BConduc'][$fila_x]> 1) 
							$BConduc = ':';
						} 
					...
					$pdf->Cell(8,$linea, $nota_aux.$BConduc ,1,0,'C'); 	
