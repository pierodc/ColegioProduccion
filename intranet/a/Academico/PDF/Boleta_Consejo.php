...
$linea = 6;
$tipologia = 'Arial';
$pdf=new FPDF('P', 'mm', array(180,130));
$SW_NewPage = true;
...
do {
	...
	//$pdf->Cell(170,$linea,'T.A.',0,1,'L'); 
	//$pdf->Cell(170,$linea, 'Año Escolar: '.$AnoEscolar ,0,1,'R'); 
	...
	$pdf->Cell(120,$linea+1.5, $Nombre  ,'0',0,'L'); 
	
	//$pdf->Cell(27,$linea,'',0,0,'C'); 
	//$pdf->SetFont('Arial','B',10);
	$pdf->Cell(35,$linea+1.5, ''. $row_RS_Curso['NombreCompleto'] ,'0',1,'R'); 
...
//					$Matriz[definitiva][suma][$fila_x] = $_row_Nota['n'.$fila_x];
	...
	foreach (array('1er ', '2do ', '3er ') as $nom) { // Titulo Trimestre
		$pdf->Cell(12,$linea, $nom.'L' ,1,0,'C',1); 		
	}
	$pdf->Cell(16,$linea, 'Final' ,1,0,'C',1); 		
	$pdf->Ln();
	
	$pdf->Cell(55,$linea, 'Asignaturas' ,0,0,'C',1);
	
	$pdf->SetFont($tipologia,'',8);
	foreach (array('1er ', '2do ', '3er ') as $nom) { // Titulo Evaluaciones de cada Trim
		
		
		//foreach (array('Def','I') as $eva) {
			$pdf->Cell(8,$linea, 'Def' ,1,0,'C',1); 	
			$pdf->Cell(4,$linea, 'i' ,1,0,'C',1); 	
		//}	
	} 
	
	foreach (array('Def','I') as $eva) {       // Titulo Evaluaciones FINAL
			$pdf->Cell(8,$linea, $eva ,1,0,'C',1); 		
	}$pdf->Ln();
	...
	foreach (array('01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12') as $fila_x) { // Nota Evaluaciones
		...
		if($Matriz[definitiva][$fila_x] > 0 and $Matriz[definitiva][$fila_x] < 10)
			$pdf->SetTextColor(255,0,0);

		$pdf->Cell(55,$linea, $Matriz[materia][$fila_x] ,1,0,'L',1); // Materia
		...
		foreach (array(1, 2, 3) as $_Lapso) {
			foreach (array('-Def','i') as $_Eval) {
				...
				if($_Eval == "-Def"){
					...
					$ancho = 4;
					$TotalInas[$fila_x] += $nota_aux;}
				$pdf->Cell($ancho,$linea, $BConduc.$nota_aux ,1,0,'C',1); 	
		...
		$pdf->SetTextColor(0);
		$pdf->SetFont($tipologia, '' , 10);
		$pdf->Cell(8,$linea, $TotalInas[$fila_x] ,1,0,'C'); 
...
*/
	...
	$pdf->Cell(55,$linea, 'Promedio' ,1,0,'C');
	...
	if($row_RS_MatPend){ // Cambiar por busqueda en AlumXCurso
		$posX = $pdf->GetX();
		$posY = $pdf->GetY();
		$pdf->Cell(50,$linea, 'Mat. Pend: ' ,1,1,'L'); 
		$Materias_Cursa = explode(',',$row_RS_MatPend['Materias_Cursa']);		
		
		$texto = $Materias_Cursa[0];
		
		$pdf->Cell(25,$linea, $texto ,1,0,'L'); 	
		$pdf->Cell(8.3,$linea, str_replace('*','',$Matriz[1][mp]) ,1,0,'C',1); 	
		$pdf->Cell(8.3,$linea, str_replace('*','',$Matriz[2][mp]) ,1,0,'C',1); 	
		$pdf->Cell(8.4,$linea, str_replace('*','',$Matriz[3][mp]) ,1,1,'C',1); 	
...
/*		
	$pdf->SetFont($tipologia,'B',13);	
	$pdf->Cell(16,$linea*1.5, 'Final ' ,0,0,'R'); 		
	...
	*/
	...
	$fotoPapa = '../../../Foto_Repre/'.$row_RS_Alumno['CodigoAlumno'].'p.jpg';
	$fotoMama = '../../../Foto_Repre/'.$row_RS_Alumno['CodigoAlumno'].'m.jpg';
	if (file_exists($fotoPapa)) {
		$pdf->Image($fotoPapa,120,20,25,0);}

	if (file_exists($fotoMama)) {
		$pdf->Image($fotoMama,150,20,25,0);}

	if (file_exists($foto)) {
		$pdf->Image($foto,120,55,0,55);}
