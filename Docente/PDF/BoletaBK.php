...
foreach($Listado as $Alumno) 	{	
	...
	$pdf->SetXY(30,40);
	$pdf->Cell(190,$linea*1.5, "Informe del Desempeño Académico" ,0,0,'C');
	$pdf->SetXY(30,48);
	$pdf->Cell(190,$linea*1.5, "del $LapsoG º Lapso" ,0,0,'C');
	...
	do{
		...
		if($Materia_Grupo_Ante != $Materia_Grupo){ 
			...
			$pdf->Ln(4);
			if($EscalaNota == 5){
				$pdf->Cell(165,$linea*1.5, $Materia_Grupo ,$Borde,0,'L');			
				...
				$pdf->Cell(190,$linea*1.5, $Materia_Grupo ,$Borde,1,'L');}
		}
	...
	// Cierra promedio materia
		...
		$pdf->SetX(110);
		$pdf->SetFont('Arial','B',14);
		$pdf->Cell(100,$linea*1.5, "Promedio Boleta:  ". $Promedio_Boleta."  "  ,"BR",0,'R'); //. $pdf->GetY()
		$pdf->Ln();
	}
